...
for (qw(password email name firstname lastname phone fax address1 address2 city state zipcode country notes shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptocontact shiptophone shiptofax shiptoemail)){
...
sub parts_links {
  ...
  $form{cart} = $db->query("
  	...
  	WHERE cart_id = ?
  	AND qty <> 0
  	ORDER BY p.partnumber", $form{cart_id}
  )->map_hashes('parts_id');
...
sub finish {
   if ($user->login){
      ...
      if ($form{shiptoname}){
        ...
        $sth = $dbh->prepare($query) or die($query);
        $sth->execute(
		  $customer_id, $form{shiptoname}, $form{shiptoaddress1}, $form{shiptoaddress2},
		  $form{shiptocity}, $form{shiptostate}, $form{shiptozipcode}, $form{shiptocountry},
		  $form{shiptocontact}, $form{shiptophone}, $form{shiptofax}, $form{shiptoemail}
        );
      }
   ...
   $sth = $dbh->prepare($query);
   $sth->execute(
	$form{sonumber}, $form{current_date}, $customer_id,
	$amount+$taxamount, $amount, $form{defaultcurrency},
	$form{notes}, $trans_id
   ) or die($query);
   ...
   if ($form{shiptoname}){
      ...
      $sth = $dbh->prepare($query) or die($query);
      $sth->execute(
	  $trans_id, $form{shiptoname}, $form{shiptoaddress1}, $form{shiptoaddress2},
	  $form{shiptocity}, $form{shiptostate}, $form{shiptozipcode}, $form{shiptocountry},
	  $form{shiptocontact}, $form{shiptophone}, $form{shiptofax}, $form{shiptoemail}
      ) or die($query);
   }
